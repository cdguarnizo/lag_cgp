function [model, options, params] = ioftcmgpOptimise(model, display, iters)
...
params = ioftcmgpExtractParam(model); 
options = optOptions;
if display
    options(1) = 1;
...
if strcmp(func2str(optim), 'optimiMinimize')
    ...
    params = optim('ioftcmgpObjectiveGradient', params, options, model);
elseif strcmp(func2str(optim), 'rprop')
    params = optim(params, 'ioftcmgpObjectiveGradient', iters, model);
else
    % NETLAB style optimization.
    [params, options] = optim('ioftcmgpObjective', params,  options, ...
        'ioftcmgpGradient', model);
end
